...
TweenService:Create(isOldGeneration,TWEEN_FAST,{Position=localValue and toggleOnPosition or toggleOffPosition,BackgroundColor3=localValue and GREEN or WHITE}):Play()
end
...
pcall(createDoorEsp,room,numberValue-50)
end
end
end
for localObject,localValue in pairs(doorEspEntries) do
...
local objectiveNames={TimerLever="Lever",KeyObtain="Key"}
...
local entityNames={BackdoorLookman="Lookman",BackdoorRush="Blitz",GlitchCube="Glitch Fragment"}
local alwaysShowEntities={BackdoorRush=true}
local proxyEntities={BackdoorLookman=true,BackdoorRush=true}
...
local closetNames={Backdoor_Wardrobe=true}
...
local itemNames={StarVial="Starlight Vial",StarBottle="Starlight Bottle"}
...
local function setupVacuumBypass(mainFrame)
task.spawn(function()
while true do
task.wait(0.5)
if isOldGeneration() then break end
...
if localObject.Name=="SideroomSpace" then
local collision=localObject:FindFirstChild("Collision")
if collision then
for _,ti in ipairs(collision:GetChildren()) do
...
local function setupLookmanBypass(mainFrame)
runService.Heartbeat:Connect(function()
if isOldGeneration() then return end
if mainFrame.get() then
if workspace:FindFirstChild("BackdoorLookman") then remotesFolder.MotorReplication:FireServer(-650) end
end
end)
end
...
local function setupTimerDisplay(mainFrame)
local timerBox=createInstance("TextLabel",espGui)
timerBox.AnchorPoint=Vector2.new(0.5,0)
timerBox.Position=UDim2.new(0.5,0,0,10)
timerBox.Size=UDim2.new(0,25,0,25)
timerBox.BackgroundColor3=darkBackground
timerBox.Font=sourceSansFont
timerBox.TextSize=20
timerBox.TextColor3=WHITE
timerBox.Text=""
timerBox.AutomaticSize=Enum.AutomaticSize.X
timerBox.Visible=false
addCorner(timerBox,6)
local padding=Instance.new("UIPadding")
padding.PaddingLeft=UDim.new(0,6)
padding.PaddingRight=UDim.new(0,6)
padding.Parent=timerBox
runService.Heartbeat:Connect(function()
if isOldGeneration() then return end
if not mainFrame.get() then timerBox.Visible=false return end
local bestLabel,bestNumber=nil,-1
for _,room in ipairs(currentRooms:GetChildren()) do
local numberValue=tonumber(room.Name)
if numberValue and numberValue>bestNumber then
local door=room:FindFirstChild("Door")
local display=door and door:FindFirstChild("DisplayTimer",true)
local textLabel=display and display:FindFirstChild("Text",true)
if textLabel and textLabel:IsA("TextLabel") then bestLabel=textLabel bestNumber=numberValue end
end
end
if bestLabel then timerBox.Text=bestLabel.ContentText timerBox.Visible=true
else timerBox.Visible=false end
end)
end
...
while p do
if p.Name=="DrawerContainer" then category="drawers" break end
p=p.Parent
end
end
if not category then
...
local function setupCamera(fa,speedToggle)
local mainGame=require(LocalPlayer.PlayerGui.MainUI.Initiator.Main_Game)
local savedFov,featureToggle=70,false
runService.RenderStepped:Connect(function()
if isOldGeneration() then return end
local localValue=fa.get()
local cameraOrCorner=workspace.CurrentCamera
...
local mainFeatures={"speed","fly","noclip","door reach","bypass","auto interact","anticheat manipulation"}
local rowHeight=50
local expandedHeights={[1]=140,[2]=95,[5]=115,[6]=175}
local bypassFeatures={"vacuum","lookman"}
...
local visualFeatures={"camera","ambient","fullbright","esp","thirdperson","timer"}
local visualExpandedHeights={[1]=120,[4]=265}
local visualRows={}
local visualRowHeights={rowHeight,rowHeight,rowHeight,rowHeight,rowHeight,rowHeight}
local mainRows={}
local mainRowHeights={rowHeight,rowHeight,rowHeight,rowHeight,rowHeight,rowHeight,rowHeight}
local bypassToggles={}
...
TweenService:Create(rootOrRow,TWEEN_FAST,{Position=UDim2.new(0,0,0,yOffset)}):Play()
yOffset=yOffset+rz[j]+5
end
TweenService:Create(contentScroller,TWEEN_FAST,{CanvasSize=UDim2.new(0,0,0,yOffset-5)}):Play()
end
...
TweenService:Create(gradientOrGlobals,TWEEN_FAST,{Rotation=expanded and 180 or 0,ImageColor3=expanded and GREEN or WHITE}):Play()
rz[instance]=expanded and eh[instance] or rowHeight
TweenService:Create(rootOrRow,TWEEN_FAST,{Size=UDim2.new(1,-20,0,rz[instance])}):Play()
layoutRows(rs,rz)
task.wait(0.2)
cooldown=false
end)
...
if not isBypass then featureToggle=temporaryValue(rootOrRow,expandedHeights[instance] and UDim2.new(1,-77,0,19) or UDim2.new(1,-44,0,19)) end
...
setupVacuumBypass(bypassToggles.vacuum)
setupLookmanBypass(bypassToggles.lookman)
...
if name=="thirdperson" or name=="fullbright" or name=="ambient" or name=="timer" then visualToggle=temporaryValue(rootOrRow,UDim2.new(1,-44,0,19)) end
...
local noCameraShakeFeature=localObject(rootOrRow,90,"no camera shake",30)
setupCamera(fovFeature,noCameraShakeFeature)
...
setupTimerDisplay(visualToggle)
end
end
